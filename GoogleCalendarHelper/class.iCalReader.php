...
{
$actualtime = time();																																/// geaenderte Version - neu eingefuegt - aktuelle Zeit
$array = $this->cal;
$events = $array['VEVENT'];
...
// Get Count
$count = (isset($rrules['COUNT']) && $rrules['COUNT'] != '') ? $rrules['COUNT'] : 0;																/// geaenderte Version - neu eingefuegt
// Get Until
//$until = $this->iCalDateToUnixTimestamp($rrules['UNTIL']);																						/// original
$until = (isset($rrules['UNTIL']) && $rrules['UNTIL'] != '') ? $this->iCalDateToUnixTimestamp($rrules['UNTIL']) : $actualtime + (30 * 86400);		/// geaenderte Version - falls kein until, dann bis + 1 Monat
...
// If there is an COUNT instead of UNTIL
if ($count > 0){$offset_raw = ($count-1)*$interval; $offsetcount = "+$offset_raw day";  $until = strtotime($offsetcount, $start_timestamp);}  	/// geaenderte Version - falls COUNT statt UNTIL angegeben ist
...
// If there is an COUNT instead of UNTIL
if ($count > 0){$offset_raw = ($count-1)*$interval; $offsetcount = "+$offset_raw week";  $until = strtotime($offsetcount, $start_timestamp); $i = 1;  	/// geaenderte Version - falls COUNT statt UNTIL
// Step through weeks
while ($week_recurring_timestamp <= $until && $i < $count) {
// Add events for bydays
$day_recurring_timestamp = $week_recurring_timestamp;
foreach ($weekdays as $day) {
// Check if day should be added
if (in_array($day, $bydays) && $day_recurring_timestamp > $start_timestamp && $day_recurring_timestamp <= $until && $i < $count) {
// Add event to day
...
else{  																																					/// geaenderte Version - falls kein COUNT
// Step through weeks
...
$recurring_timestamp = strtotime($offset, $start_timestamp);
// If there is an COUNT instead of UNTIL
if ($count > 0){$offset_raw = ($count-1)*$interval; $offsetcount = "+$offset_raw month";  $until = strtotime($offsetcount, $start_timestamp);}  	/// geaenderte Version - falls COUNT statt UNTIL angegeben ist
...
// HACK: Exchange doesn't set a correct UNTIL for yearly events, so just go 10 years out
$until = (isset($rrules['UNTIL']) && $rrules['UNTIL'] != '') ? $this->iCalDateToUnixTimestamp($rrules['UNTIL']) : strtotime('+10 year',$start_timestamp);		/// geaenderte Version - falls kein until, dann bis + 10 Jahre vom Startzeitpunkt aus

//$until = strtotime('+10 year',$start_timestamp);																									/// geaenderte Version - auf 10 Jahre erweitert
// If there is an COUNT instead of UNTIL
if ($count > 0){$offset_raw = ($count-1)*$interval; $offsetcount = "+$offset_raw year";  $until = strtotime($offsetcount, $start_timestamp);}  	/// geaenderte Version - falls COUNT statt UNTIL angegeben ist
...
$start_time = date('His',$start_timestamp);																									/// geaenderte Version - neu, damit "leere" Zeiten erkannt werden
// Step throuhg years adding specific month dates
while ($recurring_timestamp <= $until) {
//$event_start_desc = "$day {$month_names[$rrules['BYMONTH']]} ".date('Y',$recurring_timestamp)." ".date('H:i:s',$recurring_timestamp);     ///  original
//$event_start_timestamp = strtotime($event_start_desc);																				    ///  original
$event_start_desc = date('Ymd',$recurring_timestamp);																						///  geaenderte Version
$event_start_timestamp = strtotime($event_start_desc);																				        ///  geaenderte Version
if ($event_start_timestamp > $start_timestamp && $event_start_timestamp < $until) {
$anEvent['DTSTART'] = date('Ymd\T',$event_start_timestamp).$start_time;
$anEvent['DTEND'] = date('Ymd\THis',$this->iCalDateToUnixTimestamp($anEvent['DTSTART'])+$event_timestamp_offset);
$events[] = $anEvent;
